...
$toggle_nav_top.click(function () {
    ...
    if($('.sidebar-wrapper').hasClass('close_icon')){
        localStorage.setItem('close_icon', false)
    }else{
        localStorage.setItem('close_icon', true)
    }
...
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    ...
    $(".sidebar-wrapper nav").find("li").removeClass("active");
    var current = window.location
    firstPath = location.pathname.split('/')[2]
    $(".sidebar-wrapper nav ul>li a").filter(function () {
        var link = $(this).attr("href");
        if (link) {
            if (current == link) {
                $(this).parents().children('.sidebar-link').addClass('active');
                $(this).parents().parents().children('ul').css('display', 'block');
                $(this).addClass('active');
                $(this).parent().parent().parent().children('a').find('div').replaceWith('<div class="according-menu"><i class="fas fa-angle-down"></i></div>');
                $(this).parent().parent().parent().parent().parent().children('a').find('div').replaceWith('<div class="according-menu"><i class="fas fa-angle-down"></i></div>');
                return false;
            }

            if(firstPath == link.split('/')[4]){
                $(this).parents().children('.sidebar-link').addClass('active');
                $(this).parents().parents().children('ul').css('display', 'block');
                $(this).parent().parent().parent().children('a').find('div').replaceWith('<div class="according-menu"><i class="fas fa-angle-down"></i></div>');
                $(this).parent().parent().parent().parent().parent().children('a').find('div').replaceWith('<div class="according-menu"><i class="fas fa-angle-down"></i></div>');
    ...
    $(".sidebar-list>.link-nav").filter(function () {
        var link = $(this).attr("href");
        if (link) {
            if (current == link) {
                $(this).parents().children('a').addClass('active');
